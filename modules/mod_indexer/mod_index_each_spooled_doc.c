...
static int discard_error_document(spool_t *spool, void *ptr, int len)
{
	uint32_t docid;
...
static int get_first_indexwords_from_spool(uint32_t docid, 
			void **spooled_memory, int *spooled_memory_size)
{
	...
	rv = discard_error_document(spool, ptr, len);
...
int index_each_spooled_doc(uint32_t docid, word_hit_t *wordhit,
							uint32_t wordhit_size, uint32_t *idx)
{
	...
	rv = get_first_indexwords_from_spool(docid, &ptr, &len);
	if (rv != SUCCESS) {
		return rv;
	}

	indexwords = ptr + 4;
	num_of_indexwords = (len-4) / sizeof(index_word_t);
...
static void register_hooks(void)
{
	...
	sb_hook_index_each_spooled_doc(index_each_spooled_doc, NULL, post, HOOK_MIDDLE);
#if FORWARD_INDEX==1
...
module index_each_spooled_doc_module = {
