...
static int morphological_analyze(uint32_t docid, void *pCdmData, long cdmLength, void **pRmasData, long *rmasLength)
{
	int svrID;
	int i;
    char escape_buffer[LONG_STRING_SIZE];
...
#define RMAS_RETRY 5
	...
	for( i = 0; rmas_retry <= 0 || i < rmas_retry; ) {
        ...
        client = rmas_addr[svrID].client;
        if(client == NULL) {
            rmas_addr[svrID].client = sb_run_http_client_new(
                                         rmas_addr[svrID].address,
                                         rmas_addr[svrID].port);
            if ( rmas_addr[svrID].client == NULL ) {
		...
		if ( snprintf(path, LONG_STRING_SIZE,
					 "/document/ma?metadata=%s", escape_path(meta_data, escape_buffer) ) <= 0 ){
			error("query to long, max[%d]", LONG_STRING_SIZE);
			return FAIL;
		}
...
static void set_ip_and_port(configValue v)
{
	...
	memset(&rmas_addr[num_of_rmas], 0x00, sizeof(server_t));
