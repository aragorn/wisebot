...
static int register_document( memfile*  buf )
{
	...
	if ( memfile_read( buf, (char*)&total_cnt, recv_data_size) != recv_data_size ){
		error("incomplete result document count ");
        return FAIL;
	}

    info("recved document count[%d]", total_cnt);
    if( total_cnt == 0 ) {
        return SUCCESS;
    }

	setproctitle("softbotd: %s register documents[%d]", __FILE__, total_cnt);
    ...
    for( i = 0; i < total_cnt; i++ ) {
		...
		// 문서 크기 읽기
		recv_data_size = sizeof(uint32_t);
		if ( memfile_read( buf, (char*)&doc_size, recv_data_size) != recv_data_size ){
			error("incomplete result document size[%d]", recv_data_size);
			return FAIL;
		}

		info("document size[%d]", doc_size);

        // 문서 읽기
		if ( memfile_read( buf, canned_doc, doc_size) != doc_size ){
			error("incomplete result document[%d]", doc_size);
			return FAIL;
		}
        canned_doc[doc_size] = '\0';

        // OID 추출을 위한 xml 파싱
		p = sb_run_xmlparser_parselen("CP949", canned_doc, doc_size);
		if ( p == NULL ) {
...
static int replication_main(slot_t *slot)
{
	...
	while (1) { /* endless while loop */
		...
		if ( snprintf(path, MAX_QUERY_STRING_SIZE, 
					  "/replication/get_document?current_did=%u", 
					  last_registered_docid) <= 0 )
		{
        ...
        if( client->http->response_http_code != 200 ) {
            error("server response code[%d]", client->http->response_http_code);
            goto sleep;
        }
...
static void set_timeout(configValue v)
{
    ...
    DEBUG("timer is set to %d", timeout_sec);
}
