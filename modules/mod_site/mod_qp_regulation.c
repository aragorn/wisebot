...
static int docattr_filtering(docattr_cond_t *cond, char *attrquery)
{
	...
	if (attrquery[0] != '\0') {
		...
		get_str_item(buf, attrquery, "gubun:", '&', STRING_SIZE);
		if (buf[0]) {
			if (sb_run_docattr_set_doccond_function(cond, "gubun", buf) == -1) {
				return -1;
			}
		}

		get_str_item(buf, attrquery, "enactdate:", '&', STRING_SIZE);
		if (buf[0]) {
			if (sb_run_docattr_set_doccond_function(cond, "enactdate", buf) == -1) {
				return -1;
			}
		}

		get_str_item(buf, attrquery, "enfodate:", '&', STRING_SIZE);
		if (buf[0]) {
			if (sb_run_docattr_set_doccond_function(cond, "enfodate", buf) == -1) {
				return -1;
			}
		}
