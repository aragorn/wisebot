...
static int docattr_filtering(docattr_cond_t *cond, char *attrquery)
{
	...
	if (attrquery[0] != '\0') {
		...
		get_str_item(buf, attrquery, "System:", '&', STRING_SIZE);
		if (buf[0]) {
			if (sb_run_docattr_set_doccond_function(cond, "System", buf) == -1) {
				return -1;
			}
		}

		get_str_item(buf, attrquery, "Type:", '&', STRING_SIZE);
		if (buf[0]) {
			if (sb_run_docattr_set_doccond_function(cond, "Type", buf) == -1) {
				return -1;
			}
		}
	}
