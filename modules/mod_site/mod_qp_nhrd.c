...
static int docattr_filtering(docattr_cond_t *cond, char *attrquery)
{
	...
	if (attrquery[0] != '\0') {
		...
		get_str_item(buf, attrquery, "Cate1:", '&', STRING_SIZE);
		if (buf[0]) {
			if (sb_run_docattr_set_doccond_function(cond, "Cate1", buf) == -1) {
				return -1;
			}
		}

		get_str_item(buf, attrquery, "Cate2:", '&', STRING_SIZE);
		if (buf[0]) {
			if (sb_run_docattr_set_doccond_function(cond, "Cate2", buf) == -1) {
				return -1;
			}
		}
	}
