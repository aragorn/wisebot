/* $Id$ */
/*
 * http_util.c: string utility things
 * 
 * 3/21/93 Rob McCool
...
{
	debug("Done with config file %s", cfp->name);
...
#endif /* WIN32 || OS2 */
		error("Access to file %s denied by server: not a regular file", name);
        apr_file_close(file);
        return APR_EBADF;
    }
    ...
    int(*close_func)(void *param))
{
    ...
    debug("opening config handler %s", descr);
    new_cfg->param = param;
    new_cfg->name = descr;
    new_cfg->getch = getch;
...
/*
 * Unescapes a URL.
 * Returns 0(DECLINE) on success, non-zero on error
 * Failure is due to
...
#define MAXHOSTNAMELEN 256
...
#endif
    {
		warn("gethostname() failed to determine ServerName");
    }
    else 
    {
        str[sizeof(str) - 1] = '\0';
	...
	alert("could not determine the server's fully qualified "
		  "domain name, using %s for ServerName", server_hostname);
