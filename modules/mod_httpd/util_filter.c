			   ...
			   filter_trie_node * child, int c)
{
	...
	new = (filter_trie_child_ptr *) apr_palloc(p, parent->size *
						   sizeof
						   (filter_trie_child_ptr));
	memcpy(new, parent->children,
	       parent->nchildren * sizeof(filter_trie_child_ptr));
	parent->children = new;
    }
					 ...
					 filter_trie_node * parent, char c)
{
    ...
    new_node->children = 
		(filter_trie_child_ptr *) apr_palloc(p,
											 new_node->
											 size *
											 sizeof
											 (filter_trie_child_ptr));
					...
					filter_trie_node ** reg_filter_set)
{
    ...
    sb_str_tolower(normalized_name);
						       ...
						       ftype)
{
    ...
    debug("register input filter: (%s).", name);
							...
							ftype)
{
    ...
    debug("register outputfilter: (%s).", name);
					  ...
					  ap_filter_t ** c_filters)
{
	...
	debug("add filter handle : (%s)", frec->name);
	...
	if (r) {
		...
		error("a content filter was added without a request: %s",
			 frec->name);
	    return NULL;
	}
    }
	...
	if (r) {
		...
		error("a protocol filter was added without a request: %s",
			   frec->name);
	    return NULL;
	}
    }
				   ...
				   ap_filter_t ** c_filters)
{
    ...
    error("an unknown filter was not added: %s", name);
    return NULL;
}
...
static void remove_any_filter(ap_filter_t * f, ap_filter_t ** r_filt,
			      ap_filter_t ** p_filt, ap_filter_t ** c_filt)
{
