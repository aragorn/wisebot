...
BEGIN {
    ...
    printf(" * that libtool will link all of the SB functions\n")
    ...
    printf("\n")
    printf("const void *sb_ugly_hack = NULL;\n")
    printf("\n")
...
function add_symbol(symbol) {
    if (!index(symbol, "#")) {
        push("const void *sb_hack_" symbol " = (const void *)" symbol ";")
        scope_used[scope] = 1
    }
}
...
/^[ \t]*SB_DECLARE_HOOK[^(]*[(][^)]*/ {
    ...
    add_symbol("sb_hook_" symbol)
#    add_symbol("sb_hook_get_" symbol)
    add_symbol("sb_run_" symbol)
    next
}
