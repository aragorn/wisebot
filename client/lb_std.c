...
FILE *STDopenFP(char *pszFileNM, char *pszMode)
{
	...
	fpFile = fopen(pszFileNM, pszMode);
...
TBOOL	STDreadFPNum(FILE *fpFile, TINT *pnNum)
{
	...
	if ( fscanf(fpFile, "%ld", (long int *)pnNum) == EOF )
		return 0;
...
{
	fseek(fpFile, 0L, SEEK_SET);
	if ( fprintf(fpFile, "%ld", (long int)nNum) == EOF )
		return 0;
...
/* *************************************************************** */
char*  STDmakeDateToTimeStr(CToday *pToday)
{
	return NULL;
...
/* added by woosong for site restriction 2001.07.20. */
void URLtoSITE(char *url, char *site)
{
	int i, j;
	int len = strlen(url);
	j = 0;
	...
	if (url[i] == '/')
		if (url[i+1] == '~')
			for(site[j++] = url[i++]; i < len && url[i] != '/'; i++, j++)
				site[j] = url[i];
	site[j] = 0;
}
