...
<%!
public int getParameter(HttpServletRequest r, String name, int def)
{
...
public String getParameter(HttpServletRequest r, String name, String def)
{
	String value = r.getParameter(name);
...
<%
String cvs_id = "$Id$";
String target = getParameter(request, "target", "http://localhost:3000/search/search");
String query  = getParameter(request, "query",  "");
String xsl    = getParameter(request, "xsl",    "");
String submit = getParameter(request, "submit", "");
String param_name  = getParameter(request, "param_name",  "");
String param_value = getParameter(request, "param_value", "");
String download    = getParameter(request, "download",    "");
String result;
...
if ( "ok".equalsIgnoreCase(submit) )
{
	String escaped_query       = URLEncoder.encode(query);
	String escaped_param_name  = URLEncoder.encode(param_name);
	String escaped_param_value = URLEncoder.encode(param_value);
	String output;

	HttpClient client = new HttpClient();
	String requestUrl = target + "?q=" + escaped_query;
	...
	try {
		...
		if (statusCode == HttpStatus.SC_OK)
		{
			...
			System.err.println("responseBody.length=" + output.length());
		} else {
			output = "Method failed: " + method.getStatusLine();
		}
	} catch (HttpException e) {
		output = "Fatal protocol violocation: " + e.getMessage();
	} catch (IOException e) {
		output = "Fatal transport error: " + e.getMessage();
	} finally {
...
원심이 피고 성남시에 대하여 위와 같은 이유로 불법행위 책임의 성립을 인정하여 이 사건 냉해로 인한 손해배상 책임을 인정한 것은 다음과 같은 이유로 수긍하기 어렵다.
  원심 판시 자체에 의하더라도 피고 성남시는, 약정에 기하여 부담하는 '1996. 10.경까지 새로운 용수공급시설 공사를 완료하여 줄 채무'를 불이행한 것에 불과하다 할 것이고, 어떠한 위법한 행위를 하였다는 것이 아니므로, 채무불이행 책임을 지게 됨은 별론으로 하고, 위법한 행위로 타인에게 손해를 가한 경우에만 인정되는 불법행위 책임을 진다고는 볼 수 없을 뿐만 아니라(나아가 기록을 살펴보아도 피고 성남시가 1996. 6. 11.경 원고들에게 위와 같은 새로운 용수공급시설을 같은 해 10.까지 완공하여 주기로 약정하였다는 점에 부합하는 증거로는 원심에
...
삼한지 9초판 1쇄 인쇄 2006년 3월 6일 | 초판 1쇄 발행 2006년 3월 10일지은이 김정산 | 펴낸이 김태영상무 신화섭 | 편집장
 박선영 | 책임편집 양은하기획편집 1팀 이효선 도은주 성화현 | 2팀 오유미 가정실 | 3팀 최혜진 정지연 한수미디자인 김정숙 하은혜 차기윤
  ...
  (02)704-3861 | 팩스 (02)704-3891전자우편 yedam1\@wisdomhouse.co.kr | 홈페이지 www.yedamco.co.kr출력 엔터 | 종이 화인페이퍼 | 인쇄·제 본 (주) 현문값 8, 500원ISBN 89-5913-143-1 04810ISBN 89-5913-134-2 (전10권)R잘못된 책은 바꿔드립니다.R이 책의 내용과 편집 체재의 무단 전재 및 복제를 금합니다.
...
<%
	out.println("<address> $Id$ </address> </html>");
}
